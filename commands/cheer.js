module.exports.run = async (client, msg, args, db) => {
  ...
  try {
    ...
    if (!doc.exists) {
      console.log(
        "User has not been added to the cheers table, creating . . ."
      );
      ...
      console.log(
        "User exists in cheers data - incrementing stored cheer count!"
      );
    ...
    console.log(
      "error occurred while querying for user - this is unexpected. . ."
    );
    console.error(e);
  }
...
async function setCheerCount(data, msg, userData) {
  var randomQuote =
    messagePrefix[Math.floor(Math.random() * Math.floor(messagePrefix.length))];
  ...
  if (res.error) {
    ...
    msg.channel.send(
      randomQuote +
        " - " +
        msg.mentions.members.first().user.toString() +
        " has " +
        data.cheerCount +
        suffixStr
    );
